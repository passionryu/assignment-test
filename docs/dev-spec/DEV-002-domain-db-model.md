...
| Topic | Decision |
| --- | --- |
| 회원가입/로그인 | Lv1에서는 구현하지 않는다. seed member와 `X-Member-Id` 기반 데모 인증으로 제한한다. |
| 추억 이미지 | 실제 파일 업로드는 구현하지 않는다. 이미지 URL 문자열과 seed dummy image 경로로 대체한다. |
| 미션 인증 | DB는 여러 submission을 저장할 수 있게 두되, MVP 화면과 목록은 최신 submission을 현재 인증으로 사용한다. |
| 회원 탈퇴 후 기록 | 회원은 soft delete 처리하고 기존 기록은 유지한다. 화면 표시는 `탈퇴한 사용자`로 대체한다. |
| 비밀번호 변경 | Lv1에서는 API 계약과 최소 검증을 제공한다. 운영 수준의 인증/세션 보안은 후속 범위로 둔다. |
