...
PM/PO Agent는 작업을 Human QA로 넘기기 전에 이 채팅방에서 반드시 아래 2가지를 보고한다.

1. 뭘 구현했는지
   - API, DB, 프론트엔드, 문서 변경을 사용자가 이해할 수 있는 단위로 요약한다.
   - 브랜치, 커밋, 실행 URL, 자동 검증 결과가 있으면 함께 적는다.
   - 구현하지 않은 범위나 의도적으로 제외한 범위가 있으면 명시한다.

2. 뭘 QA 하면 되는지
   - 사용자가 직접 클릭, 입력, 확인해야 할 항목을 체크리스트로 제시한다.
   - 화면, API, 경계 케이스, 화면 정의서 일치 여부를 필요한 만큼 구분한다.
   - Human QA에서 실패하면 어느 단계로 되돌릴지 함께 적는다.

Human QA 인계 메시지 기본 형식:
...
- Human QA로 넘기기 전 `구현한 내용`과 `Human QA 체크리스트`가 이 채팅방에 보고됐다.
- 제출 전 남은 P0 작업이 없다.
...
- 차단 사항
- 다음에 호출할 Agent
- Human QA 인계 시 `구현한 내용`
- Human QA 인계 시 `Human QA 체크리스트`
