...
class LocalSeedRunner(
    ...
    private fun seedBookOrders() {
        jdbcTemplate.update(
            """
            delete from print_order_status_histories where order_id between 9001 and 9020;
            delete from print_order_contents where order_id between 9001 and 9020;
            ...
            values
                ...
                (
                    ...
                    'QA 진행 주문 - 제작 중',
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '1 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '25 days')::date,
                    72,
                    32000,
                    9600,
                    3000,
                ...
                (
                    ...
                    'QA 진행 주문 - 제작 완료',
                    3,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '3 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '18 days')::date,
                    64,
                    28000,
                    3080,
                    3000,
                ...
                (
                    ...
                    'QA 진행 주문 - 배송 중',
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '5 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '8 days')::date,
                    42,
                    46000,
                    840,
                    3000,
                ...
                (
                    ...
                    'QA 진행 주문 - 가족 사진집',
                    2,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '2 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '12 days')::date,
                    86,
                    28000,
                    7920,
                    3000,
                ...
                (
                    ...
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '5 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '27 days')::date,
                    118,
                    32000,
                    23400,
                    3000,
                ...
                (
                    ...
                    'QA 완료 주문 - 하드커버 추억북',
                    2,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '2 months')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '40 days')::date,
                    60,
                    46000,
                    8400,
                    3000,
                ...
                (
                    ...
                    '지훈 주문 - 제작 중',
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '4 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '24 days')::date,
                    96,
                    32000,
                    16800,
                    3000,
                ...
                (
                    ...
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '8 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '19 days')::date,
                    34,
                    46000,
                    0,
                    3000,
                    ...
                    now() - interval '12 hours 40 minutes',
                    now() - interval '12 hours 40 minutes',
                    '여행 준비방 콘텐츠를 더 모은 뒤 다시 주문'
                ),
                (
                    9019,
                    'BO-SEED-9019',
                    ...
                    '가족 여행 사진방 주문 - 배송 중',
                    1,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '20 days')::date,
                    (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '3 days')::date,
                    48,
                    46000,
                    3360,
            ...
            values
                ...
                (900801, 9008, 'MEMORY', 301, '가족 여행 출발', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '1 days')::date, 3, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 301, 'title', '가족 여행 출발', 'description', '가족 여행 첫날 사진과 댓글을 담은 주문 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '1 days')::date::text, 'authorName', '류성열', 'imageCount', 3, 'commentCount', 4, 'pageCount', 3, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (900802, 9008, 'LETTER', 401, '가족에게 남긴 편지', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '3 days')::date, 2, 2, jsonb_build_object('type', 'LETTER', 'sourceId', 401, 'title', '가족에게 남긴 편지', 'description', '가족 여행 뒤 고마움을 남긴 편지', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '3 days')::date::text, 'authorName', '류성열', 'imageCount', 0, 'commentCount', 0, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '편지')::text),
                (900901, 9009, 'MISSION', 112, '프로젝트 결과물 인증', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '8 days')::date, 4, 1, jsonb_build_object('type', 'MISSION', 'sourceId', 112, 'title', '프로젝트 결과물 인증', 'description', '실습 결과물과 승인 댓글이 많은 미션 인증', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '8 days')::date::text, 'authorName', '지훈', 'imageCount', 2, 'commentCount', 6, 'pageCount', 4, 'selectedByDefault', true, 'sourceLabel', '미션 인증')::text),
                (900902, 9009, 'CHAT', 208, '프로젝트 회의 채팅 묶음', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '9 days')::date, 5, 2, jsonb_build_object('type', 'CHAT', 'sourceId', 208, 'title', '프로젝트 회의 채팅 묶음', 'description', '회의 당일 논의한 채팅을 하루 단위로 묶은 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '9 days')::date::text, 'authorName', '류성열', 'imageCount', 0, 'commentCount', 18, 'pageCount', 5, 'selectedByDefault', true, 'sourceLabel', '채팅 묶음')::text),
                (901001, 9010, 'MEMORY', 305, '기념일 후보 사진', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '3 days')::date, 3, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 305, 'title', '기념일 후보 사진', 'description', '배송 중 주문의 대표 추억 게시글', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '3 days')::date::text, 'authorName', '류성열', 'imageCount', 2, 'commentCount', 2, 'pageCount', 3, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901002, 9010, 'LETTER', 405, '기념일 전날 편지', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '1 days')::date, 2, 2, jsonb_build_object('type', 'LETTER', 'sourceId', 405, 'title', '기념일 전날 편지', 'description', '배송 중 주문에 포함된 비공개 편지', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '1 days')::date::text, 'authorName', '여자친구', 'imageCount', 0, 'commentCount', 0, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '편지')::text),
                (901101, 9011, 'MEMORY', 310, '가족 저녁 식탁', ((now() at time zone 'Asia/Seoul')::date - 2), 3, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 310, 'title', '가족 저녁 식탁', 'description', '새로 주문 요청한 가족 사진집 콘텐츠', 'occurredDate', ((now() at time zone 'Asia/Seoul')::date - 2)::text, 'authorName', '아버지', 'imageCount', 2, 'commentCount', 3, 'pageCount', 3, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901102, 9011, 'MISSION', 111, '장보기 인증', ((now() at time zone 'Asia/Seoul')::date - 1), 2, 2, jsonb_build_object('type', 'MISSION', 'sourceId', 111, 'title', '장보기 인증', 'description', '가족 미션 인증을 포함한 주문', 'occurredDate', ((now() at time zone 'Asia/Seoul')::date - 1)::text, 'authorName', '류성열', 'imageCount', 1, 'commentCount', 2, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '미션 인증')::text),
                (901201, 9012, 'MEMORY', 320, '발표 리허설 사진', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '10 days')::date, 4, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 320, 'title', '발표 리허설 사진', 'description', 'A4 프로젝트 기록집에 들어가는 사진 많은 게시글', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '10 days')::date::text, 'authorName', '지훈', 'imageCount', 4, 'commentCount', 5, 'pageCount', 4, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901202, 9012, 'CHAT', 236, '마감 전 채팅 묶음', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '12 days')::date, 6, 2, jsonb_build_object('type', 'CHAT', 'sourceId', 236, 'title', '마감 전 채팅 묶음', 'description', '마감 전날 정리한 채팅 묶음', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '12 days')::date::text, 'authorName', '류성열', 'imageCount', 0, 'commentCount', 28, 'pageCount', 6, 'selectedByDefault', true, 'sourceLabel', '채팅 묶음')::text),
                (901301, 9013, 'MEMORY', 330, '100일 여행 앨범', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '55 days')::date, 5, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 330, 'title', '100일 여행 앨범', 'description', '배송 완료된 하드커버 추억북의 대표 사진 페이지', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '55 days')::date::text, 'authorName', '류성열', 'imageCount', 5, 'commentCount', 4, 'pageCount', 5, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901302, 9013, 'LETTER', 410, '100일 기념 편지', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '53 days')::date, 2, 2, jsonb_build_object('type', 'LETTER', 'sourceId', 410, 'title', '100일 기념 편지', 'description', '완료 주문의 기념일 편지', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '53 days')::date::text, 'authorName', '여자친구', 'imageCount', 0, 'commentCount', 0, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '편지')::text),
                (901401, 9014, 'MISSION', 114, '가족 책장 정리', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '4 days')::date, 2, 1, jsonb_build_object('type', 'MISSION', 'sourceId', 114, 'title', '가족 책장 정리', 'description', '취소 주문에 남아 있는 미션 인증 스냅샷', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '4 days')::date::text, 'authorName', '아버지', 'imageCount', 1, 'commentCount', 1, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '미션 인증')::text),
                (901501, 9015, 'MEMORY', 340, '페이지 초과 후보 묶음', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '9 days')::date, 7, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 340, 'title', '페이지 초과 후보 묶음', 'description', '오류 주문의 페이지 검수 실패 상황을 보여주는 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '9 days')::date::text, 'authorName', '류성열', 'imageCount', 6, 'commentCount', 8, 'pageCount', 7, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901601, 9016, 'MEMORY', 350, '부모님 산책 사진', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '18 days')::date, 3, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 350, 'title', '부모님 산책 사진', 'description', '아버지 계정의 배송 완료 주문 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '18 days')::date::text, 'authorName', '아버지', 'imageCount', 2, 'commentCount', 2, 'pageCount', 3, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901701, 9017, 'MISSION', 118, '팀 노트 인증', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '11 days')::date, 3, 1, jsonb_build_object('type', 'MISSION', 'sourceId', 118, 'title', '팀 노트 인증', 'description', '지훈 계정의 제작 중 주문 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '11 days')::date::text, 'authorName', '지훈', 'imageCount', 1, 'commentCount', 3, 'pageCount', 3, 'selectedByDefault', true, 'sourceLabel', '미션 인증')::text),
                (901801, 9018, 'MEMORY', 360, '여행 준비 체크리스트', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '9 days')::date, 2, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 360, 'title', '여행 준비 체크리스트', 'description', '여자친구 계정의 취소 주문 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date + interval '9 days')::date::text, 'authorName', '여자친구', 'imageCount', 1, 'commentCount', 1, 'pageCount', 2, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (901901, 9019, 'MEMORY', 370, '가족 여행 바다 사진', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '13 days')::date, 4, 1, jsonb_build_object('type', 'MEMORY', 'sourceId', 370, 'title', '가족 여행 바다 사진', 'description', '가족 여행 사진방 배송 중 주문 콘텐츠', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '13 days')::date::text, 'authorName', '아버지', 'imageCount', 4, 'commentCount', 5, 'pageCount', 4, 'selectedByDefault', true, 'sourceLabel', '추억 게시글')::text),
                (902001, 9020, 'CHAT', 245, '학급 공지 채팅 묶음', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '8 days')::date, 4, 1, jsonb_build_object('type', 'CHAT', 'sourceId', 245, 'title', '학급 공지 채팅 묶음', 'description', '4학년 1반 주문 오류 확인용 채팅 묶음', 'occurredDate', (date_trunc('month', now() at time zone 'Asia/Seoul')::date - interval '8 days')::date::text, 'authorName', '지훈', 'imageCount', 0, 'commentCount', 22, 'pageCount', 4, 'selectedByDefault', true, 'sourceLabel', '채팅 묶음')::text);
            ...
            values
                ...
                (910019, 9008, null, 'PAID', 1, 'seed 주문 생성', now() - interval '1 day 8 hours'),
                (910020, 9008, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '1 day 5 hours'),
                (910021, 9008, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '1 day 2 hours'),
                (910022, 9008, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '20 hours'),
                (910023, 9009, null, 'PAID', 1, 'seed 주문 생성', now() - interval '1 day 4 hours'),
                (910024, 9009, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '1 day'),
                (910025, 9009, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '18 hours'),
                (910026, 9009, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '14 hours'),
                (910027, 9009, 'IN_PRODUCTION', 'PRODUCTION_COMPLETE', 100, 'seed 제작 완료', now() - interval '10 hours'),
                (910028, 9010, null, 'PAID', 1, 'seed 주문 생성', now() - interval '2 days 6 hours'),
                (910029, 9010, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '2 days'),
                (910030, 9010, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '1 day 18 hours'),
                (910031, 9010, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '1 day 12 hours'),
                (910032, 9010, 'IN_PRODUCTION', 'PRODUCTION_COMPLETE', 100, 'seed 제작 완료', now() - interval '8 hours'),
                (910033, 9010, 'PRODUCTION_COMPLETE', 'SHIPPED', 100, 'seed 배송 중', now() - interval '3 hours'),
                (910034, 9011, null, 'PAID', 1, 'seed 주문 생성', now() - interval '50 minutes'),
                (910035, 9012, null, 'PAID', 1, 'seed 주문 생성', now() - interval '35 minutes'),
                (910036, 9012, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '20 minutes'),
                (910037, 9013, null, 'PAID', 1, 'seed 주문 생성', now() - interval '18 days'),
                (910038, 9013, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '17 days'),
                (910039, 9013, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '16 days'),
                (910040, 9013, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '15 days'),
                (910041, 9013, 'IN_PRODUCTION', 'PRODUCTION_COMPLETE', 100, 'seed 제작 완료', now() - interval '14 days'),
                (910042, 9013, 'PRODUCTION_COMPLETE', 'SHIPPED', 100, 'seed 배송 중', now() - interval '12 days'),
                (910043, 9013, 'SHIPPED', 'DELIVERED', 100, 'seed 배송 완료', now() - interval '11 days'),
                (910044, 9014, null, 'PAID', 1, 'seed 주문 생성', now() - interval '9 days'),
                (910045, 9014, 'PAID', 'CANCELLED_REFUND', 1, '취소 사유: 수량을 2권으로 다시 주문하기 위해 취소', now() - interval '8 days 23 hours'),
                (910046, 9015, null, 'PAID', 1, 'seed 주문 생성', now() - interval '6 days'),
                (910047, 9015, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '5 days 22 hours'),
                (910048, 9015, 'PDF_READY', 'ERROR', 100, 'seed 페이지 검수 실패', now() - interval '5 days 18 hours'),
                (910049, 9016, null, 'PAID', 3, 'seed 아버지 주문 생성', now() - interval '12 days'),
                (910050, 9016, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '11 days'),
                (910051, 9016, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '10 days'),
                (910052, 9016, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '9 days'),
                (910053, 9016, 'IN_PRODUCTION', 'PRODUCTION_COMPLETE', 100, 'seed 제작 완료', now() - interval '7 days'),
                (910054, 9016, 'PRODUCTION_COMPLETE', 'SHIPPED', 100, 'seed 배송 중', now() - interval '6 days'),
                (910055, 9016, 'SHIPPED', 'DELIVERED', 100, 'seed 배송 완료', now() - interval '5 days'),
                (910056, 9017, null, 'PAID', 4, 'seed 지훈 주문 생성', now() - interval '16 hours'),
                (910057, 9017, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '12 hours'),
                (910058, 9017, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '9 hours'),
                (910059, 9017, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '6 hours'),
                (910060, 9018, null, 'PAID', 2, 'seed 여자친구 주문 생성', now() - interval '13 hours'),
                (910061, 9018, 'PAID', 'CANCELLED_REFUND', 2, '취소 사유: 여행 준비방 콘텐츠를 더 모은 뒤 다시 주문', now() - interval '12 hours 40 minutes'),
                (910062, 9019, null, 'PAID', 3, 'seed 가족 여행 사진방 주문 생성', now() - interval '4 days'),
                (910063, 9019, 'PAID', 'PDF_READY', 100, 'seed 제작 파일 준비', now() - interval '3 days 18 hours'),
                (910064, 9019, 'PDF_READY', 'CONFIRMED', 100, 'seed 주문 확정', now() - interval '3 days 8 hours'),
                (910065, 9019, 'CONFIRMED', 'IN_PRODUCTION', 100, 'seed 제작 중', now() - interval '2 days 12 hours'),
                (910066, 9019, 'IN_PRODUCTION', 'PRODUCTION_COMPLETE', 100, 'seed 제작 완료', now() - interval '1 day 12 hours'),
                (910067, 9019, 'PRODUCTION_COMPLETE', 'SHIPPED', 100, 'seed 배송 중', now() - interval '1 days'),
                (910068, 9020, null, 'PAID', 4, 'seed 학급 주문 생성', now() - interval '3 days 4 hours'),
                (910069, 9020, 'PAID', 'ERROR', 100, 'seed 학급 주문 오류', now() - interval '3 days');
            """.trimIndent(),
        )
    }
